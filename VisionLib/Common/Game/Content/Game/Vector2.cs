...
namespace VisionLib.Common.Game.Content.Game
{
	/// <summary>
	...
	public class Vector2
	{
		...
		{
			return Angle(this, new Vector2(targetX, targetY));
		}

		public static double Angle(Vector2 v1, Vector2 v2, bool clamp = true)
		{
			var num = Math.Atan2(v2.Y - v1.Y, v2.X - v1.X) * MathUtils.Rad2Deg;
			if (clamp && num < 0.0)
				num += 360.0;
			return num;
		}
		...
		{
			var num = Math.Atan2(-v2.Y + v1.Y, v2.X - v1.X) * MathUtils.Rad2Deg + 90.0;
			if (num < 0.0)
				num += 360.0;
			return num;
		}
		...
		{
			var num = angle * MathUtils.Deg2Rad;
		...
		public static void Copy(Vector2 v1, ref Vector2 v2)
		{
			if (v2 == null)
			{
				v2 = new Vector2(v1);
